...
public class DatasetService {
	...
	public Path getDataset() throws IOException {
		...
		String[] headers = { "ModelName", "NodeName", "BehaviorIndex", "ActionType", "ActionOrder", "Topic", "Value",
				"ExecutionTime" };
				...
				for (Node node : model.getNodes()) {
					...
					for (Behavior behavior : node.getBehaviors()) {
						...
						for (Action action : behavior.getActions()) {
							...
							if (y == 0 || action.getType().equals("pub") || action.getExecutionTimes().isEmpty()
									|| action.getExecutionTimes().size() <= y) {
								row[7] = "0";
							} else if (action.getType().equals("timer")) {
								row[7] = String.format(Locale.US, "%.5f",
										action.getExecutionTimes().get(y) - action.getExecutionTimes().get(y - 1));
							} else if (action.getType().equals("sub")) {
								row[7] = String.format(Locale.US, "%.5f", action.getExecutionTimes().get(y)
										- executionPubMap.get(action.getTopic()).get(y));
							}
