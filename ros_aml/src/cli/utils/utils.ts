...
export function getBehaviorMethodName(behavior: Behavior): string {
    if (isTimerElapsed(behavior.trigger))
        return `on_timerElapsed_${behavior.trigger.timer}`;
    if (isMessageReceived(behavior.trigger))
        return `on_messageReceived_${behavior.trigger.topic}`;
    if (isServiceRequest(behavior.trigger))
        return `on_serviceRequest_${behavior.trigger.service}`;
    if (isActionGoalReceived(behavior.trigger))
        return `on_actionGoalReceived_${behavior.trigger.action}`;
    if (isParamChanged(behavior.trigger))
        return `on_paramChanged_${behavior.trigger.param}`;
    if (isStateChanged(behavior.trigger))
        return `on_stateChanged_${behavior.trigger.state}`;
...
export function log_with_timer(
    wantLog: boolean,
    exec: CompositeGeneratorNode,
    name_timer: string,
    level: string = 'info',
    action_type: string = 'pub',
    b_index: number = 0,
    a_index: number = 0
): void {
    if (!wantLog) {
    ...
    exec.appendNewLine();
    exec.append(
        `        self.get_logger().${level}("${action_type} ${b_index} ${a_index} " + str(${name_timer}_time))`
    );
    exec.appendNewLine();
}
